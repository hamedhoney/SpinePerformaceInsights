...
def describe_data(_):
    ...
    return (
        "Here is a statistical summary of the first 10 numeric columns in your dataset:\n\n"
        f"```\n{summary.to_string()}\n```"
    )
...
def query_average(question: str):
    ...
    if match:
        ...
        if level in st.session_state.df['Level'].unique():
            val = st.session_state.df[st.session_state.df['Level'] == level]['SuperiorResultant'].mean()
            return f"The average load at {level} is {val:.2f}"
